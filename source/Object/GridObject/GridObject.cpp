...
GridObject::GridObject() : Object()
{
	...
	shaderProgram = ShaderLoader::Load("grid.vs.glsl", NULL, NULL, NULL, "grid.fs.glsl");
}
...
void GridObject::generateGrid()
{
	...
	{
		if (i == 0 && Settings::ShowAxes.active)
		{
			this->vertices.insert(this->vertices.end(), {(float)i, 0.0f, -halfGrid, 1.0f, 1.0f, 1.0f,
			                                             (float)i, 0.0f, 0.0f, 1.0f, 1.0f, 1.0f});

			this->vertices.insert(this->vertices.end(), {-halfGrid, 0.0f, (float)i, 1.0f, 1.0f, 1.0f,
			                                             0.0f, 0.0f, (float)i, 1.0f, 1.0f, 1.0f});
		}
	...
	glBufferData(GL_ARRAY_BUFFER, this->vertices.size() * sizeof(GLfloat), this->vertices.data(),
	             GL_STATIC_DRAW);

	glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
	glBufferData(GL_ELEMENT_ARRAY_BUFFER, this->indices.size() * sizeof(GLuint), this->indices.data(),
	             GL_STATIC_DRAW);
